...
function computePseudoRefSol_DW(y, nobs=21)
...
function plotResults(enkfFile,mlenkfFile)
    ...
    figure(2)
    f(t) = log.(10 .+t).^(1/3) .* t.^(-1. /2);
    loglog(tML, errMML, "k", tML, (1.2*errMML[end]/f(tML[end]))*f(tML), "k--",  tEnkf, errMEnkf, "k-o",  tEnkf, (1.2*errMEnkf[end]*tEnkf[end]^(1/3))*tEnkf.^(-1. /3), "k--o")
    xlabel("Runtime [s]");    ylabel("RMSE");
    savefig("loglogRatesRmse_Mean"*enkfFile[5:end-4]*".eps", bbox_inches="tight");
    
    
    figure(3)
    
    loglog(tML, 1errCML, "k",tML, (1.2*errCML[end]/f(tML[end]))*f(tML), "k--", tEnkf, errCEnkf, "k-o",  tEnkf, (1.2*errCEnkf[end]*tEnkf[end]^(1/3)) *tEnkf.^(-1. /3), "k--o")
    xlabel("Runtime [s]");    ylabel("RMSE");
